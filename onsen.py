...
def prettify_table(table, separator=" "):
    ...
    line_list = [
        separator.join( \
            "{0}".format(x) if i == len(line) - 1 else "{0:{width}}".format(x, width=max_width[i]) \
...
def list_all(soup):
    ...
    # prepare report text
    table = [("ID", "Name")] + [(li["id"], li.find("h4").get_text()) for li in content]
    # prettify table
...
def list_daily(soup, day):
    ...
    content = soup.select(".listWrap li")
    content = [x for x in content if x["data-week"] == day_selector]

    # prepare report text
    table = [("Status", "ID", "Name")]

    for li in content:
        status = "Update" if "newChecked" in li["class"] else "Normal"
...
def download_audio(id, proxy=None):
    try:
        respose = urllib.request.urlopen("http://www.onsen.ag/data/api/getMovieInfo/{id}".format(id=id), timeout=60)
    ...
    print(("Downloading audio: {0}".format(path)), end=' ')
    try:
        if not proxy:
            pass
            #import urllib
...
def download_images(soup, id):
    ...
    url = "http://www.onsen.ag/program/{id}/".format(id=id)
...
def process(option):
    ...
    if option.sp_name == "list":
        try:
            soup = BeautifulSoup(urllib.request.urlopen("http://www.onsen.ag/", timeout=60), "html.parser")
        except Exception as e:
            handle_error(e, "Network Error.")
            return
        ...
        if option.everything or option.image:
            try:
                soup = BeautifulSoup(urllib.request.urlopen("http://www.onsen.ag/program/{id}".format(id=option.id), timeout=30), "html.parser")
            except Exception as e:
                handle_error(e, "Network Error.")
                return
...
if __name__ == "__main__":
    ...
    action.add_argument("-d", "--day", action="store", dest="day", nargs=1, metavar="day", type=int, choices=list(range(1,8)), help="bangumi of specifical day")
